...
class Admin::ImagesController < Admin::BaseController
  ...
  def index
      ...
      if params[:start]
  		...
  		@end_time = params[:end] || Time.zone.now.to_s(:db)
  	else
  		@start_time = Time.zone.now.strftime("%Y-%m-%d") + " 00:00:00"
  		@end_time = Time.zone.now.to_s(:db)
  	end
  ...
  def mark
  	...
  	water_logger.debug("[begin][#{Time.zone.now}] **********begin***********")

  	mark_file = "public/watermarks/#{params[:mark][:water]}"
  	water_logger.debug("[info][#{Time.zone.now.to_s(:db)}] watermark file is '#{mark_file}'")
  	...
  	@goods.each do |good|
  		good.images.each do |image|
  			...
  			if File.exist?(src)
                        ...
                        `cp #{src} #{src}.#{Time.zone.now.to_i}`
                        water_logger.debug("[info][#{Time.zone.now.to_s(:db)}] file '#{src}' backup")
  				begin
		  			...
		  			composited = origin.composite(mark,SouthEastGravity,10,10,OverCompositeOp)
		  	  		...
		  	  		# resized.write(middle)
		  	      		water_logger.debug("[info][#{Time.zone.now.to_s(:db)}][#{good.bn}] file '#{src}' composited.")
  				rescue Exception => e
  					water_logger.debug("[error][#{Time.zone.now.to_s(:db)}][#{good.bn}] file '#{src}'  #{e}.")
  				end
	  	    	else
	  	    		water_logger.debug("[info][#{Time.zone.now.to_s(:db)}][#{good.bn}] file '#{src}' is not exists.")
	  	    	end

  		end
  	end
  	water_logger.debug("[end][#{Time.zone.now.to_s(:db)}] *********end***********")
  ...
  def rollback
    ...
    @goods = Ecstore::Good.where("uptime >= ? and uptime < ?",Time.parse(@start_time).to_i,Time.parse(@end_time).to_i)
    water_logger.debug("[begin][#{Time.zone.now}] **********begin rollbacking***********")
    @goods.each do |good|
        good.images.each do |image|
          src = "#{ECSTORE_ROOT_PATH}/#{image.m_url}"
          if File.exists?(src)
              ...
              if origin_id
                ...
                water_logger.debug("[info][#{Time.zone.now}][#{good.bn}] #{src} is rollback,Message=#{result}")
              else
                water_logger.debug("[info][#{Time.zone.now}][#{good.bn}] #{src} is already original file")
              end
          else
                water_logger.debug("[info][#{Time.zone.now}][#{good.bn}] #{src} is not exists")
          end
        end
    end
     water_logger.debug("[begin][#{Time.zone.now}] **********end rollback***********")
