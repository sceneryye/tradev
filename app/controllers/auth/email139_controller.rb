...
class Auth::Email139Controller < ApplicationController
	...
	def index 
		...
		time2000 = Time.parse('2000-01-01').to_i
		timestamp = Time.now.to_i - time2000 +10

		xml_hash={}
		xml_hash[:SessionKey] = sid
		xml_hash[:ComeFrom] = 'test'
	    ...
	    xml_hash[:Skey]  = Digest::MD5.hexdigest(unsign).upcase
 		...
 		#return render :text=> xml
      	...
      	url = 'http://121.15.167.240:19090/SSOInterface/GetUserByKey'
     	RestClient.get(url)
	    ...
	    if res_data_hash['ResponseData']['RetCode']=='000'
	    	...
	    	#验证返回
	      	unsign_hash = Hash.send :[],  res_data_hash['ResponseData'].select{ |key,val| val.present? && key != 'TimeStamp' && key!='Skey'}
	        unsign = unsign_hash.collect do |key,val| 	"#{val}" end.join("") + key
	       # 	return render :text=>unsign
	        if Digest::MD5.hexdigest(unsign) != res_data_hash['ResponseData']['Skey']
	        	return render :text=>'返回数据未通过验证'
	        end
	    	
	        user_number =res_data_hash['ResponseData']['UserNumber']
	     #   return render :text=>res_data_hash
	 #       redirect_to "/auth/email139/#{sid}/callback?timestamp=#{res_data_hash['ResponseData']['TimeStamp']}&user_number=#{user_number}"
	        # 登录或注册
	    else
	     ...
	     #    return render :text=>"#{xml}||#{res_data_hash}||返回码：[#{res_data_hash['ResponseData']['RetCode']}]\r\n
